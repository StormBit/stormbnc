...
qwebirc.ui.ConfirmBox = function(parentElement, callback, initialNickname, initialChannels, autoNick, networkName) {
  ...
  text.appendChild(document.createTextNode("To connect to " + networkName + " and join channel" + ((c.length>1)?"s":"") + " "));
...
qwebirc.ui.LoginBox = function(parentElement, callback, initialNickname, initialChannels, networkName) {
  ...
  var td = new Element("td");
  tr.appendChild(td);
  td.set("html", "<h1>Connect to " + networkName + "</h1>");  
  ...
  if(qwebirc.auth.enabled()) {
    if(qwebirc.auth.passAuth()) {
      ...
      if(qwebirc.auth.bouncerIdent()) {
        var identBox = new Element("input");
        createRow("BNC Network:", identBox);
      }
      var passwordRow = createRow("BNC Password:");
  ...
  form.addEvent("submit", function(e) {
    ...
    if(qwebirc.auth.enabled()) {
      if(qwebirc.auth.passAuth() && authCheckBox.checked) {
        ...
        if(qwebirc.auth.bouncerIdent()) {
          if(!identBox.value) {
            alert("You must supply a BNC username.");
            identBox.focus();
            return;
          }

          data["ident"] = identBox.value;
        }
        if(!passwordBox.value) {
          alert("You must supply a BNC password.");
          passwordBox.focus();
          return;
        }
