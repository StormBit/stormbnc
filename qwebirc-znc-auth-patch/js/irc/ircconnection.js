...
qwebirc.irc.IRCConnection = new Class({
  ...
  connect: function() {
    ...
    if($defined(this.options.ident))
      postdata+="&ident=" + encodeURIComponent(this.initialNickname + '/' + this.options.ident);
    if($defined(this.options.serverPassword))
      postdata+="&password=" + encodeURIComponent(this.initialNickname + '/' + this.options.ident + ':' + this.options.serverPassword);      
    r.send(postdata);
  },
